...
<div class="center">
```{r, echo=FALSE, label=urna2,out.width = "650px"}
knitr::include_graphics("04Interpolacion_files/weierstrass.png",dpi=400)
```
</div>
...
for (i in 1:length(nodos)){
  aux=nodos[-i]
  denom=1
  pp=polymul(c(1,-aux[1]),c(1,-aux[2]))
  denom=(nodos[i]-aux[1])*(nodos[i]-aux[2])
  for (j in 3:length(aux)){
    pp=polymul(pp,c(1,-aux[j]))
    denom=denom*(nodos[i]-aux[j])
  }
  polis.lag=rbind(polis.lag,pp)  
  denoms=c(denoms,denom)
}
signe=function(x){ifelse(x>0,"+",ifelse(x<0,"-",""))}
signe1=function(x){ifelse(x>0,"","-")}
```
...
$$
\begin{array}{rl}
L_{3,0}(x) = & \frac{(x-1)\cdot (x-3)\cdot (x-5)}{(0-1)\cdot (0-3)\cdot (0-5)}=\frac{1}{`r denoms[1]`}\cdot (x-1)\cdot (x-3)\cdot (x-5)=\frac{1}{`r denoms[1]`}\cdot (x^3`r signe(polis.lag[1,2])``r abs(polis.lag[1,2])` x^2`r signe(polis.lag[1,3])``r abs(polis.lag[1,3])`x`r signe(polis.lag[1,4])``r abs(polis.lag[1,4])`),\\
L_{3,1}(x)= &  \frac{x\cdot (x-3)\cdot (x-5)}{(1-0)\cdot (1-3)\cdot (1-5)}=\frac{1}{`r denoms[2]`}\cdot x\cdot (x-3)\cdot (x-5)=\frac{1}{`r denoms[2]`}\cdot (x^3`r signe(polis.lag[2,2])``r abs(polis.lag[2,2])` x^2`r signe(polis.lag[2,3])``r abs(polis.lag[2,3])`x),\\
L_{3,2}(x)= &  \frac{x\cdot (x-1)\cdot (x-5)}{(3-0)\cdot (3-1)\cdot (3-5)}=\frac{1}{`r denoms[3]`}\cdot x\cdot (x-1)\cdot (x-5)=\frac{1}{`r denoms[3]`}\cdot (x^3`r signe(polis.lag[3,2])``r abs(polis.lag[3,2])` x^2`r signe(polis.lag[3,3])``r abs(polis.lag[3,3])`x),\\
L_{3,3}(x)= &  \frac{x\cdot (x-1)\cdot (x-3)}{(5-0)\cdot (5-1)\cdot (5-3)}=\frac{1}{`r denoms[4]`}\cdot x\cdot (x-1)\cdot (x-3)=\frac{1}{`r denoms[4]`}\cdot (x^3`r signe(polis.lag[4,2])``r abs(polis.lag[4,2])` x^2`r signe(polis.lag[4,3])``r abs(polis.lag[4,3])`x).
\end{array}
$$
...
<div class="center">
```{r, echo=FALSE, label=urna3,out.width = "650px"}
knitr::include_graphics("04Interpolacion_files/lagrange.png",dpi=400)
```
</div>
...
$$
(`r nodos[1]`,`r f(nodos[1])`),\ (`r nodos[2]`,`r f(nodos[2])`),\ (`r nodos[3]`,`r f(nodos[3])`),\ (`r nodos[4]`,`r f(nodos[4])`)
$$
El polinomio interpolador será:
$$
\begin{array}{rl}
P_3(x) = & `r y[1]`\cdot \left(\frac{1}{`r denoms[1]`}\cdot (x^3`r signe(polis.lag[1,2])``r abs(polis.lag[1,2])` x^2`r signe(polis.lag[1,3])``r abs(polis.lag[1,3])`x`r signe(polis.lag[1,4])``r abs(polis.lag[1,4])`)\right)`r signe(y[2])``r abs(y[2])`\cdot\left(\frac{1}{`r denoms[2]`}\cdot (x^3`r signe(polis.lag[2,2])``r abs(polis.lag[2,2])` x^2`r signe(polis.lag[2,3])``r abs(polis.lag[2,3])`x)\right)\\ &
`r signe(y[3])``r abs(y[3])`\cdot\left(\frac{1}{`r denoms[3]`}\cdot (x^3`r signe(polis.lag[3,2])`
`r abs(polis.lag[3,2])` x^2`r signe(polis.lag[3,3])``r abs(polis.lag[3,3])`x)\right)
`r signe(y[4])``r abs(y[4])`\cdot\left(\frac{1}{`r denoms[4]`}\cdot (x^3`r signe(polis.lag[4,2])`
`r abs(polis.lag[4,2])` x^2`r signe(polis.lag[4,3])``r abs(polis.lag[4,3])`x)\right)\\
= & `r coefs[1]`\cdot x^3`r signe(coefs[2])``r abs(coefs[2])`\cdot x^2`r signe(coefs[3])``r abs(coefs[3])`\cdot x.
\end{array}
$$
...
<div class="center">
```{r, echo=FALSE, label=urna4,out.width = "650px"}
knitr::include_graphics("04Interpolacion_files/lagrange2.png",dpi=400)
```
</div>
...
$$
f(x)-P_3(x) = \frac{\pi^3}{`r 16*factorial(4)`}\left(-8 \cos\left(\frac{\pi}{2} \xi(x)\right)+\pi \xi(x) \sin\left(\frac{\pi}{2} \xi(x)\right)\right)\cdot x\cdot (x-1)\cdot (x-3)\cdot (x-5).
$$
...
```{r,echo=FALSE}
nodos=c(0,1,3,5)
pp=polymul(c(1,-nodos[1]),c(1,-nodos[2]))
for (j in 3:length(nodos)){
  pp=polymul(pp,c(1,-nodos[j]))
}
pp.der=pp*c(4:0)
pp.der=rev(pp.der)
pp.der=pp.der[-1]
ceros=polyroot(pp.der)
ceros=Re(ceros)
gg=function(x){abs(prod(x-nodos))}
```
<div class="example">

Imaginemos que queremos acotar el error cometido $|f(x)-P_3(x)|$ para todo valor de $x$ en el intervalo $[0,5]$. Entonces, tendremos:
$$
|f(x)-P_3(x)|\leq  \frac{\pi^3 (8+5\pi)}{`r 16*factorial(4)`}\cdot\max_{x\in [0,5]}|x\cdot (x-1)\cdot (x-3)\cdot (x-5)|.
$$

Para calcular el valor $\max_{x\in [0,5]}|x\cdot (x-1)\cdot (x-3)\cdot (x-5)|$ tenemos que derivar la función $h(x)=x\cdot (x-1)\cdot (x-3)\cdot (x-5)=x^4`r signe(pp[2])``r abs(pp[2])`x^3`r signe(pp[3])``r abs(pp[3])`x^2`r signe(pp[4])``r abs(pp[4])`x$ ya que el valor máximo de la misma se alcanzará en el interior (en los extremos, $h(0)=h(5)=0$):
$$
h'(x)=4x^3`r signe(pp[2])``r abs(3*pp[2])`x^2`r signe(pp[3])``r abs(2*pp[3])`x`r signe(pp[4])``r abs(pp[4])`.
$$
...
$$
\begin{array}{rl}
g(`r ceros[1]`)= & `r gg(ceros[1])`,\\
g(`r ceros[2]`)= & `r gg(ceros[2])`,\\
g(`r ceros[3]`)= & `r gg(ceros[3])`.
\end{array}
$$
Entonces, $\displaystyle \max_{x\in [0,5]}|x\cdot (x-1)\cdot (x-3)\cdot (x-5)|=`r max(gg(ceros[1]),gg(ceros[2]),gg(ceros[3]))`$.

El error cometido al interpolar $f(x)$ en los nodos anteriores para $x\in [0,5]$ se puede acotar por:
$$
|f(x)-P_3(x)|\leq \frac{\pi^3 (8+5\pi)}{`r 16*factorial(4)`}\cdot`r max(gg(ceros[1]),gg(ceros[2]),gg(ceros[3]))`\approx `r pi^3*(8+5*pi)*max(gg(ceros[1]),gg(ceros[2]),gg(ceros[3]))/(16*factorial(4))`.
$$
...
$$
\begin{array}{rl}
L_{2,0}(x) = & \frac{(x-3)\cdot (x-5)}{(0-3)\cdot (0-5)}=\frac{1}{`r denoms[1]`}\cdot (x-3)\cdot (x-5)=\frac{1}{`r denoms[1]`}\cdot (x^2`r signe(polis.lag[1,2])``r abs(polis.lag[1,2])` x`r signe(polis.lag[1,3])``r abs(polis.lag[1,3])`),\\
L_{2,1}(x)= & \frac{x\cdot (x-5)}{(3-0)\cdot (3-5)}=\frac{1}{`r denoms[2]`}\cdot x\cdot (x-5)=\frac{1}{`r denoms[2]`}\cdot (x^2`r signe(polis.lag[2,2])``r abs(polis.lag[2,2])` x),\\
\\
L_{2,2}(x)= & \frac{x\cdot (x-3)}{(5-0)\cdot (5-3)}=\frac{1}{`r denoms[3]`}\cdot x\cdot (x-3)=\frac{1}{`r denoms[3]`}\cdot (x^2`r signe(polis.lag[3,2])``r abs(polis.lag[3,2])` x).
\end{array}
$$
El polinomio interpolador será:
$$
P_{0,2,3}(x) =  `r y[1]`\cdot \left(\frac{1}{`r denoms[1]`}\cdot (x^2`r signe(polis.lag[1,2])``r abs(polis.lag[1,2])` x `r signe(polis.lag[1,3])``r abs(polis.lag[1,3])`)\right)`r signe(y[2])``r abs(y[2])`\cdot\left(\frac{1}{`r denoms[2]`}\cdot (x^2`r signe(polis.lag[2,2])``r abs(polis.lag[2,2])` x)\right)`r signe(y[3])``r abs(y[3])`\cdot\left(\frac{1}{`r denoms[3]`}\cdot (x^2`r signe(polis.lag[3,2])`
`r abs(polis.lag[3,2])` x)\right)=  x^2`r signe(coefs[2])``r abs(coefs[2])`\cdot x.
$$
...
lag.pols = list()
nodos=c(0,1,3,5)
lag.pols[[1]]=as.matrix(c(0,1,-3,5),ncol=4)
for (i in 2:(length(nodos))){
  aux=c()
  for (j in i:length(nodos)){
    aux2=(-polymul(c(1,-nodos[j-i+1]),lag.pols[[i-1]][j-i+2,])+polymul(c(1,-nodos[j]),lag.pols[[i-1]][j-i+1,]))/(nodos[j-i+1]-nodos[j])
   aux=rbind(aux,aux2)
  }
  lag.pols[[i]]=aux
}
...
|:---:|:---|:---|:---|:---|
|$x_0=`r nodos[1]`$|$Q_{0,0}=`r lag.pols[[1]][1,]`$||||
|$x_1=`r nodos[2]`$|$Q_{1,0}=`r lag.pols[[1]][2,]`$|$\begin{array}{rl}Q_{1,1}= & \frac{(x-`r nodos[1]`) `r lag.pols[[1]][2,]`-(x-`r nodos[2]`)`r lag.pols[[1]][1,]`}{`r nodos[2]`-`r nodos[1]`}\\ =& `r lag.pols[[2]][1,1]`x\end{array}$|||
|$x_2=`r nodos[3]`$|$Q_{2,0}=`r lag.pols[[1]][3,]`$|$\begin{array}{rl} Q_{2,1}= & \frac{(x-`r nodos[2]`) (`r lag.pols[[1]][3,]`)-(x-`r nodos[3]`)`r lag.pols[[1]][2,]`}{`r nodos[3]`-`r nodos[2]`}\\ = & `r lag.pols[[2]][2,1]`x+`r lag.pols[[2]][2,2]`\end{array}$|$Q_{2,2}=`r lag.pols[[3]][1,1]`x^2`r signe(lag.pols[[3]][1,2])``r abs(lag.pols[[3]][1,2])`x$||
|$x_3=`r nodos[4]`$|$Q_{3,0}=`r lag.pols[[1]][4,]`$|$\begin{array}{rl} Q_{3,1}= & \frac{(x-`r nodos[3]`) (`r lag.pols[[1]][4,]`)-(x-`r nodos[4]`)(`r lag.pols[[1]][3,]`)}{`r nodos[4]`-`r nodos[3]`}\\ = & `r lag.pols[[2]][3,1]`x `r signe(lag.pols[[2]][3,2])``r abs(lag.pols[[2]][3,2])`\end{array}$|$Q_{3,2}=`r lag.pols[[3]][2,1]`x^2`r signe(lag.pols[[3]][2,2])``r abs(lag.pols[[3]][2,2])`x`r signe(lag.pols[[3]][2,3])``r abs(lag.pols[[3]][2,3])`$|$Q_{3,3}=`r lag.pols[[4]][1,1]`x^3`r signe(lag.pols[[4]][1,2])``r abs(lag.pols[[4]][1,2])`x^2`r signe(lag.pols[[4]][1,3])``r abs(lag.pols[[4]][1,3])`x$|
</div>
</div>
...
$$
\begin{align*}
Q_{2,2}(x)=& \frac{(x-`r nodos[1]`) Q_{2,1}(x)-(x-`r nodos[3]`) Q_{1,1}(x)}{`r nodos[3]`-`r nodos[1]`}= \frac{(x-`r nodos[1]`) (`r lag.pols[[2]][2,1]`x+`r lag.pols[[2]][2,2]`)-(x-`r nodos[3]`) (`r lag.pols[[2]][1,1]`x)}{`r nodos[3]`-`r nodos[1]`} = `r lag.pols[[3]][1,1]`x^2`r signe(lag.pols[[3]][1,2])``r abs(lag.pols[[3]][1,2])`x,\\
Q_{3,2}(x) = & \frac{(x-`r nodos[2]`) Q_{3,1}(x)-(x-`r nodos[4]`) Q_{2,1}(x)}{`r nodos[4]`-`r nodos[2]`}= \frac{(x-`r nodos[2]`) (`r lag.pols[[2]][3,1]`x`r lag.pols[[2]][3,2]`)-(x-`r nodos[4]`) (`r lag.pols[[2]][2,1]`x+`r lag.pols[[2]][2,2]`)}{`r nodos[4]`-`r nodos[2]`} \\ = &  `r lag.pols[[3]][2,1]`x^2`r signe(lag.pols[[3]][2,2])``r abs(lag.pols[[3]][2,2])`x`r signe(lag.pols[[3]][2,3])``r abs(lag.pols[[3]][2,3])`,\\
Q_{3,3}(x) = & \frac{(x-`r nodos[1]`) Q_{3,2}(x)-(x-`r nodos[4]`) Q_{2,2}(x)}{`r nodos[4]`-`r nodos[1]`}= \frac{(x-`r nodos[1]`) (`r lag.pols[[3]][2,1]`x^2`r signe(lag.pols[[3]][2,2])``r abs(lag.pols[[3]][2,2])`x`r signe(lag.pols[[3]][2,3])``r abs(lag.pols[[3]][2,3])`)-(x-`r nodos[4]`) (`r lag.pols[[3]][1,1]`x^2`r signe(lag.pols[[3]][1,2])``r abs(lag.pols[[3]][1,2])`x)}{`r nodos[4]`-`r nodos[1]`} \\ = &  `r lag.pols[[4]][1,1]`x^3`r signe(lag.pols[[4]][1,2])``r abs(lag.pols[[4]][1,2])`x^2`r signe(lag.pols[[4]][1,3])``r abs(lag.pols[[4]][1,3])`x. 
\end{align*}
$$
</div>
...
* el polinomio $Q_{1,1}(x)=`r lag.pols[[2]][1,1]`x$ interpola los puntos $(x_0=0,y_0=0)$ y $(x_1=1,y_1=1)$,
* el polinomio $Q_{2,1}(x)=`r lag.pols[[2]][2,1]`x+`r lag.pols[[2]][2,2]`$ interpola los puntos $(x_1=1,y_1=1)$ y $(x_2=3,y_2=-3)$,
* el polinomio $Q_{3,1}(x)=`r lag.pols[[2]][3,1]`x `r signe(lag.pols[[2]][3,2])``r abs(lag.pols[[2]][3,2])`$ interpola los puntos $(x_2=3,y_2=-3)$ y $(x_3=5,y_3=5)$,
* el polinomio $Q_{2,2}(x)=`r lag.pols[[3]][1,1]`x^2`r signe(lag.pols[[3]][1,2])``r abs(lag.pols[[3]][1,2])`x$ interpola los puntos $(x_0=0,y_0=0)$, $(x_1=1,y_1=1)$ y $(x_2=3,y_2=-3)$,
* el polinomio $Q_{3,2}(x)=`r lag.pols[[3]][2,1]`x^2`r signe(lag.pols[[3]][2,2])``r abs(lag.pols[[3]][2,2])`x`r signe(lag.pols[[3]][2,3])``r abs(lag.pols[[3]][2,3])`$ interpola los puntos $(x_1=1,y_1=1)$, $(x_2=3,y_2=-3)$ y $(x_3=5,y_3=5)$,
* el polinomio $Q_{3,3}(x)=`r lag.pols[[4]][1,1]`x^3`r signe(lag.pols[[4]][1,2])``r abs(lag.pols[[4]][1,2])`x^2`r signe(lag.pols[[4]][1,3])``r abs(lag.pols[[4]][1,3])`x$ es el polinomio de interpolación de todos los puntos: $(x_0=0,y_0=0)$, $(x_1=1,y_1=1)$, $(x_2=3,y_2=-3)$ y $(x_3=5,y_3=5)$.
...
|$x_i$|$Q_{i,0}$|$Q_{i,1}$|$Q_{i,2}$|$Q_{i,3}$|
|:---:|:---|:---|:---|:---|
|$`r nodos[1]`$|$`r lag.pols[[1]][1,]`$||||
|$`r nodos[2]`$|$`r lag.pols[[1]][2,]`$|$\frac{(2-`r nodos[1]`) `r lag.pols[[1]][2,]`-(2-`r nodos[2]`)`r lag.pols[[1]][1,]`}{`r nodos[2]`-`r nodos[1]`}= `r lag.pols[[2]][1,1]*2`$|||
|$`r nodos[3]`$|$`r lag.pols[[1]][3,]`$|$\frac{(2-`r nodos[2]`) (`r lag.pols[[1]][3,]`)-(2-`r nodos[3]`)`r lag.pols[[1]][2,]`}{`r nodos[3]`-`r nodos[2]`}=  `r lag.pols[[2]][2,1]*2+ lag.pols[[2]][2,2]`$|$\frac{(2-0)(-1)-(2-3)2}{3-0}=0$||
|$`r nodos[4]`$|$`r lag.pols[[1]][4,]`$|$\frac{(2-`r nodos[3]`) (`r lag.pols[[1]][4,]`)-(2-`r nodos[4]`)(`r lag.pols[[1]][3,]`)}{`r nodos[4]`-`r nodos[3]`} = `r lag.pols[[2]][3,1]*2+lag.pols[[2]][3,2]`$|$\frac{(2-1)(-7)-(2-5)(-1)}{5-1}=-2.5$|$\frac{(2-0)(-2.5)-(2-5)0}{5-0}=-1$|
</div>
</div>
...
|:---:|:---|:---|:---|:---|:---|
|$`r nodos[1]`$|$`r lag.pols[[1]][1,]`$|||||
|$`r nodos[2]`$|$`r lag.pols[[1]][2,]`$|$`r lag.pols[[2]][1,1]*2`$||||
|$`r nodos[3]`$|$`r lag.pols[[1]][3,]`$|$`r lag.pols[[2]][2,1]*2+ lag.pols[[2]][2,2]`$|$0$|||
|$`r nodos[4]`$|$`r lag.pols[[1]][4,]`$|$`r lag.pols[[2]][3,1]*2+lag.pols[[2]][3,2]`$|$-2.5$|$-1$||
...
Los valores $f(x_i)$, $i=0,1,2,3$ valen $`r y[1]`,`r y[2]`, `r y[3]`, `r y[4]`.$
...
|$x_i$|$f(x_i)$|orden $1$|orden $2$|orden $3$|
|:---:|:---:|:---|:---|:---|
|$\scriptstyle `r x[1]`$|$\scriptstyle `r y[1]`$||||
|||$\scriptstyle f[`r x[1]`,`r x[2]`]=\frac{`r y[2]`-`r y[1]`}{`r x[2]`-`r x[1]`}=`r F[2,2]`$|||
|$\scriptstyle `r x[2]`$|$\scriptstyle `r y[2]`$||$\scriptstyle f[x_0,x_1,x_2]=\frac{`r F[3,2]`-`r F[2,1]`}{`r x[3]`-`r x[1]`}=`r F[3,3]`$||
|||$\scriptstyle  f[`r x[2]`,`r x[3]`]=\frac{`r y[3]`-`r y[2]`}{`r x[3]`-`r x[2]`}=`r F[3,2]`$||$\scriptstyle f[`r x[1]`,`r x[2]`,`r x[3]`,`r x[4]`]=\frac{`r F[4,3]`+`r abs(F[3,3])`}{`r x[4]`-`r x[1]`}=`r F[4,4]`$|
|$\scriptstyle `r x[3]`$|$\scriptstyle `r y[3]`$||$\scriptstyle f[x_1,x_2,x_3]=\frac{`r F[4,2]`+`r abs(F[3,2])`}{`r x[4]`-`r x[2]`}=`r F[4,3]`$||
|||$\scriptstyle f[`r x[3]`,`r x[4]`]=\frac{`r y[4]`+`r abs(y[3])`}{`r x[4]`-`r x[3]`}=`r F[4,2]`$|||
|$\scriptstyle `r x[4]`$|$\scriptstyle `r y[4]`$||||
</div>
</div>
...
$$
\begin{align*}
P_3(x)= & `r F[1,1]``r signe(F[2,2])``r abs(F[2,2])`(x-`r x[1]`)`r signe(F[3,3])``r abs(F[3,3])`(x-`r x[1]`)(x-`r x[2]`)`r signe(F[4,4])``r abs(F[4,4])`(x-`r x[1]`)(x-`r x[2]`)(x-`r x[3]`),\\
= & x-x(x-`r x[2]`)`r signe(F[4,4])``r abs(F[4,4])`x(x-`r x[2]`)(x-`r x[3]`)
\end{align*}
$$
...
$$
P_3(x)= `r F[4,1]`+`r F[4,2]` (x-`r x[4]`)+`r F[4,3]`(x-`r x[4]`)(x-`r x[3]`)+`r F[4,4]`(x-`r x[4]`)(x-`r x[3]`)(x-`r x[2]`).
$$
...
n=5
x=-1+2*(0:n)/n
f=function(t){1/(1+(cos(pi*t/2))^2)}
y=f(x)
deltas =list(y)
for (i in 2:(n+1)){
  deltas[[i]]=deltas[[i-1]][-1]-deltas[[i-1]][-length(deltas[[i-1]])]
}
```
...
Consideremos por ejemplo $n=5$. Los nodos serán $`r x[1]`,\ `r x[2]`,\ `r x[3]`,\ `r x[4]`,\ `r x[5]`,\ `r x[6]`$.

Hallemos el polinomio interpolador usando diferencias hacia adelante.

Los valores de $\Delta^k f(x_0)=\Delta^k f(`r x[1]`)$, para $k=1,2,3,4,5$ son los siguientes:
...
$$
\begin{align*}
\Delta f(`r x[1]`)= & f(`r x[2]`)-f(`r x[1]`)=`r y[2]`-`r y[1]`=`r deltas[[2]][1]`,\\
\Delta f(`r x[2]`)= & f(`r x[3]`)-f(`r x[2]`)=`r y[3]`-`r y[2]`=`r deltas[[2]][2]`,\\
\Delta f(`r x[3]`)= & f(`r x[4]`)-f(`r x[3]`)=`r y[4]`-`r y[3]`=`r deltas[[2]][3]`,\\
\Delta f(`r x[4]`)= & f(`r x[5]`)-f(`r x[4]`)=`r y[5]`-`r y[4]`=`r deltas[[2]][4]`,\\
\Delta f(`r x[5]`)= & f(`r x[6]`)-f(`r x[5]`)=`r y[6]`-`r y[5]`=`r deltas[[2]][5]`.
\end{align*}
$$
...
$$
\begin{align*}
\Delta^2 f(`r x[1]`)= & \Delta f(`r x[2]`)-\Delta f(`r x[1]`)=`r deltas[[2]][2]`-(`r deltas[[2]][1]`)=`r deltas[[3]][1]`,\\
\Delta^2 f(`r x[2]`)= & \Delta f(`r x[3]`)-\Delta f(`r x[2]`)=`r deltas[[2]][3]`-(`r deltas[[2]][2]`)=`r deltas[[3]][2]`,\\
\Delta^2 f(`r x[3]`)= & \Delta f(`r x[4]`)-\Delta f(`r x[3]`)=`r deltas[[2]][4]`-`r deltas[[2]][3]`=`r deltas[[3]][3]`,\\
\Delta^2 f(`r x[4]`)= & \Delta f(`r x[5]`)-\Delta f(`r x[4]`)=`r deltas[[2]][5]`-(`r deltas[[2]][4]`)=`r deltas[[3]][4]`.
\end{align*}
$$
...
$$
\begin{align*}
\Delta^3 f(`r x[1]`)= & \Delta^2 f(`r x[2]`)-\Delta^2 f(`r x[1]`)=`r deltas[[3]][2]`-`r deltas[[3]][1]`=`r deltas[[4]][1]`,\\
\Delta^3 f(`r x[2]`)= & \Delta^2 f(`r x[3]`)-\Delta^2 f(`r x[2]`)=`r deltas[[3]][3]`-`r deltas[[3]][2]`=`r deltas[[4]][2]`,\\
\Delta^3 f(`r x[3]`)= & \Delta^2 f(`r x[4]`)-\Delta^2 f(`r x[3]`)=`r deltas[[3]][4]`-`r deltas[[3]][3]`=`r deltas[[4]][3]`.
\end{align*}
$$
...
$$
\begin{align*}
\Delta^4 f(`r x[1]`)= & \Delta^3 f(`r x[2]`)-\Delta^3 f(`r x[1]`)=`r deltas[[4]][2]`-`r deltas[[4]][1]`=`r deltas[[5]][1]`,\\
\Delta^4 f(`r x[2]`)= & \Delta^3 f(`r x[3]`)-\Delta^3 f(`r x[2]`)=`r deltas[[4]][3]`-`r deltas[[4]][2]`=`r deltas[[5]][2]`.
\end{align*}
$$
...
* $k=5$:
$$
\Delta^5 f(`r x[1]`)=  \Delta^4 f(`r x[2]`)-\Delta^4 f(`r x[1]`)=`r deltas[[5]][2]`-(`r deltas[[5]][1]`)=`r deltas[[6]][1]`.
$$
</div>
...
\binom{s}{4}= & \frac{s(s-1)(s-2)(s-3)}{`r factorial(4)`},\\
\binom{s}{5}= & \frac{s(s-1)(s-2)(s-3)(s-4)}{`r factorial(5)`},\\
\end{align*}
$$
...
$$
\begin{align*}
P_5(x)= & f(`r x[1]`)+s\cdot \Delta f(`r x[1]`)+\frac{s(s-1)}{2}\cdot \Delta^2 f(`r x[1]`)+\frac{s(s-1)(s-2)}{6}\cdot \Delta^3 f(`r x[1]`)\\ & + \frac{s(s-1)(s-2)(s-3)}{`r factorial(4)`}\cdot \Delta^4 f(`r x[1]`) + \frac{s(s-1)(s-2)(s-3)(s-4)}{`r factorial(5)`}\cdot \Delta^5 f(`r x[1]`) \\
= & `r y[1]``r signe(deltas[[2]][1])``r abs(deltas[[2]][1])` s`r signe(deltas[[3]][1])`\frac{`r abs(deltas[[3]][1])`}{2} s (s-1)`r signe(deltas[[4]][1])`\frac{`r abs(deltas[[4]][1])`}{6} s (s-1) (s-2)\\
& `r signe(deltas[[5]][1])`\frac{`r abs(deltas[[5]][1])`}{`r factorial(4)`} s (s-1)(s-2)(s-3)
+`r signe(deltas[[6]][1])`\frac{`r abs(deltas[[6]][1])`}{`r factorial(5)`} s (s-1)(s-2)(s-3)(s-4)\\
= & `r y[1]``r signe(deltas[[2]][1])``r abs(deltas[[2]][1])` s`r signe(deltas[[3]][1])``r abs(deltas[[3]][1])/2` s (s-1)`r signe(deltas[[4]][1])``r abs(deltas[[4]][1])/6` s (s-1) (s-2)\\
& `r signe(deltas[[5]][1])``r abs(deltas[[5]][1])/factorial(4)` s (s-1)(s-2)(s-3),
\end{align*}
$$
donde $s=\frac{x-(`r x[1]`)}{`r x[2]-x[1]`}=`r 1/(x[2]-x[1])`(x`r signe(-x[1])``r abs(-x[1])`)$.
...
nablas =list(yr)
for (i in 2:(n+1)){
  nablas[[i]]=nablas[[i-1]][-length(nablas[[i-1]])]-nablas[[i-1]][-1]
}
```
...
Los valores de $\nabla^k f(x_n)=\nabla^k f(`r xr[1]`)$, para $k=1,2,3,4,5$ son los siguientes:

* $k=1$:
$$
\begin{align*}
\nabla f(`r xr[1]`)= & f(`r xr[1]`)-f(`r xr[2]`)=`r y[1]`-`r y[2]`=`r nablas[[2]][1]`,\\
\nabla f(`r xr[2]`)= & f(`r xr[2]`)-f(`r xr[3]`)=`r y[2]`-`r y[3]`=`r nablas[[2]][2]`,\\                    \nabla f(`r xr[3]`)= & f(`r xr[3]`)-f(`r xr[4]`)=`r y[3]`-`r y[4]`=`r nablas[[2]][3]`,\\                    \nabla f(`r xr[4]`)= & f(`r xr[4]`)-f(`r xr[5]`)=`r y[4]`-`r y[5]`=`r nablas[[2]][4]`,\\                    \nabla f(`r xr[5]`)= & f(`r xr[5]`)-f(`r xr[6]`)=`r y[5]`-`r y[6]`=`r nablas[[2]][5]`. 
\end{align*}
$$

* $k=2$:                                                                                                    $$
\begin{align*}
\nabla^2 f(`r xr[1]`)= & \nabla f(`r xr[1]`)-\nabla f(`r xr[2]`)=`r nablas[[2]][1]`-`r nablas[[2]][2]`=`r nablas[[3]][1]`,\\                                                                                     \nabla^2 f(`r xr[2]`)= & \nabla f(`r xr[2]`)-\nabla f(`r xr[3]`)=`r nablas[[2]][2]`-`r nablas[[2]][3]`=`r nablas[[3]][2]`,\\                                                                                     \nabla^2 f(`r xr[3]`)= & \nabla f(`r xr[3]`)-\nabla f(`r xr[4]`)=`r nablas[[2]][3]`-(`r nablas[[2]][4]`)=`r nablas[[3]][3]`,\\                                                                                       \nabla^2 f(`r xr[4]`)= & \nabla f(`r xr[4]`)-\nabla f(`r xr[5]`)=`r nablas[[2]][4]`-(`r nablas[[2]][5]`)=`r nablas[[3]][4]`.                                                                                       \end{align*}
$$     
...
* $k=3$:                                                                                                    $$                                                                                                          \begin{align*}                                                                                              \nabla^3 f(`r xr[1]`)= & \nabla^2 f(`r xr[1]`)-\nabla^2 f(`r xr[2]`)=`r nablas[[3]][1]`-`r nablas[[3]][2]`=`r nablas[[4]][1]`,\\ 
\nabla^3 f(`r xr[2]`)= & \nabla^2 f(`r xr[2]`)-\nabla^2 f(`r xr[3]`)=`r nablas[[3]][2]`-`r nablas[[3]][3]`=`r nablas[[4]][2]`,\\
\nabla^3 f(`r xr[3]`)= & \nabla^2 f(`r xr[3]`)-\nabla^2 f(`r xr[4]`)=`r nablas[[3]][3]`-`r nablas[[3]][4]`=`r nablas[[4]][3]`.                                                                         \end{align*}
$$                                                                                                          

* $k=4$:                                                                                                  
$$
\begin{align*}                                                                                              \nabla^4 f(`r xr[1]`)= & \nabla^3 f(`r xr[1]`)-\nabla^3 f(`r xr[2]`)=`r nablas[[4]][1]`-`r nablas[[4]][2]`=`r nablas[[5]][1]`,\\
\nabla^4 f(`r xr[2]`)= & \nabla^3 f(`r xr[2]`)-\nabla^3 f(`r xr[3]`)=`r nablas[[4]][2]`-`r nablas[[4]][3]`=`r nablas[[5]][2]`.
\end{align*}
$$                                                                                                          

* $k=5$:
$$
\nabla^5 f(`r xr[1]`)=  \nabla^4 f(`r xr[1]`)-\nabla^4 f(`r xr[2]`)=`r nablas[[5]][1]`-(`r nablas[[5]][2]`)=`r nablas[[6]][1]`.
$$
...
$$
\begin{align*}
P_5(x)= & f(`r xr[1]`)-s\cdot \nabla f(`r xr[1]`)+\frac{s(s-1)}{2}\cdot \nabla^2 f(`r xr[1]`)-\frac{s(s-1)(s-2)}{6}\cdot \nabla^3 f(`r xr[1]`)\\ & + \frac{s(s-1)(s-2)(s-3)}{`r factorial(4)`}\cdot \nabla^4 f(`r xr[1]`) - \frac{s(s-1)(s-2)(s-3)(s-4)}{`r factorial(5)`}\cdot \nabla^5 f(`r xr[1]`) \\
= & `r yr[1]``r signe(-nablas[[2]][1])``r abs(nablas[[2]][1])` s`r signe(nablas[[3]][1])`\frac{`r abs(nablas[[3]][1])`}{2} s (s-1)`r signe(-nablas[[4]][1])`\frac{`r abs(nablas[[4]][1])`}{6} s (s-1) (s-2)\\
& `r signe(nablas[[5]][1])`\frac{`r abs(nablas[[5]][1])`}{`r factorial(4)`} s (s-1)(s-2)(s-3)+`r signe(nablas[[6]][1])`\frac{`r abs(nablas[[6]][1])`}{`r factorial(5)`} s (s-1)(s-2)(s-3)(s-4)\\= & `r y[1]``r signe(-nablas[[2]][1])``r abs(nablas[[2]][1])` s`r signe(nablas[[3]][1])``r abs(nablas[[3]][1])/2` s (s-1)`r signe(-nablas[[4]][1])``r abs(nablas[[4]][1])/6` s (s-1) (s-2)\\                         & `r signe(nablas[[5]][1])``r abs(nablas[[5]][1])/factorial(4)` s (s-1)(s-2)(s-3), 
\end{align*}
$$
donde $s=\frac{`r xr[1]`-x}{`r xr[1]-xr[2]`}=`r 1/(xr[1]-xr[2])` (`r xr[1]`-x).$
...
for (i in 1:length(x)){
  ...
  denom=1
  pp=polymul(c(1,-aux[1]),c(1,-aux[2]))
  denom=(x[i]-aux[1])*(x[i]-aux[2])
  for (j in 3:length(aux)){
    pp=polymul(pp,c(1,-aux[j]))
    denom=denom*(x[i]-aux[j])
  }
  polis.lag=rbind(polis.lag,pp)  
  denoms=c(denoms,denom)
}
pols.lagrange=diag(1/denoms)%*%polis.lag
Hnj=c()
Htildenj=c()
for (i in 1:length(x)){
  aux1=polymul(pols.lagrange[i,],pols.lagrange[i,])
  aux2=polymul(c(-2*horner(pols.lagrange[i,],x[i])$dy,1+2*x[i]*horner(pols.lagrange[i,],x[i])$dy),aux1)
  Hnj=rbind(Hnj,aux2)
  aux3=polymul(c(1,-x[i]),aux1)
  Htildenj=rbind(Htildenj,aux3)
}
p.hermite=diag(y)%*%Hnj+diag(y.der)%*%Htildenj
p.hermite=apply(p.hermite,2,sum)
```
<div class="center">
||$x_0$|$x_1$|$x_2$|$x_3$|
|:---:|:---:|:---:|:---:|:---:|
|$f(x_i)$|$`r y[1]`$|$`r y[2]`$|$`r y[3]`$|$`r y[4]`$|
|$f'(x_i)$|$`r y.der[1]`$|$`r y.der[2]`$|$`r y.der[3]`$|$`r y.der[4]`$|
</div>
...
$$
\begin{align*}
H_{3,0}(x)=&(1-2 x L_{n,0}'(`r x[1]`))L_{n,0}^2 (x)=(1-2x (`r round(horner(pols.lagrange[1,],x[1])$dy,4)`))\left(\frac{1}{`r denoms[1]`}\cdot (x^3`r signe(polis.lag[1,2])``r abs(polis.lag[1,2])` x^2`r signe(polis.lag[1,3])``r abs(polis.lag[1,3])`x`r signe(polis.lag[1,4])``r abs(polis.lag[1,4])`)\right)^2\\ = & `r Hnj[1,1]` x^7`r signe(Hnj[1,2])``r abs(Hnj[1,2])`x^6`r signe(Hnj[1,3])``r abs(Hnj[1,3])`x^5`r signe(Hnj[1,4])``r abs(Hnj[1,4])`x^4`r signe(Hnj[1,5])``r abs(Hnj[1,5])`x^3`r signe(Hnj[1,6])``r abs(Hnj[1,6])`x^2`r signe(Hnj[1,8])``r abs(Hnj[1,8])`,\\
H_{3,1}(x)=&(1-2 (x-`r x[2]`) L_{n,1}'(`r x[2]`))L_{n,1}^2 (x)=(1-2 (x-`r x[2]`) (`r round(horner(pols.lagrange[2,],x[2])$dy,4)`))\left(\frac{1}{`r denoms[2]`}\cdot (x^3`r signe(polis.lag[2,2])``r abs(polis.lag[2,2])` x^2`r signe(polis.lag[2,3])``r abs(polis.lag[2,3])`x)\right)^2\\ = & `r Hnj[2,1]` x^7`r signe(Hnj[2,2])``r abs(Hnj[2,2])`x^6`r signe(Hnj[2,3])``r abs(Hnj[2,3])`x^5`r signe(Hnj[2,4])``r abs(Hnj[2,4])`x^4`r signe(Hnj[2,5])``r abs(Hnj[2,5])`x^3`r signe(Hnj[2,6])``r abs(Hnj[2,6])`x^2,\\
H_{3,2}(x)=&(1-2 (x-`r x[3]`) L_{n,2}'(`r x[3]`))L_{n,2}^2 (x)=(1-2 (x-`r x[3]`) (`r round(horner(pols.lagrange[3,],x[3])$dy,4)`))\left(\frac{1}{`r denoms[3]`}\cdot (x^3`r signe(polis.lag[3,2])``r abs(polis.lag[3,2])` x^2`r signe(polis.lag[3,3])``r abs(polis.lag[3,3])`x)\right)^2\\ = & `r Hnj[3,1]` x^7`r signe(Hnj[3,2])``r abs(Hnj[3,2])`x^6`r signe(Hnj[3,3])``r abs(Hnj[3,3])`x^5`r signe(Hnj[3,4])``r abs(Hnj[3,4])`x^4`r signe(Hnj[3,5])``r abs(Hnj[3,5])`x^3`r signe(Hnj[3,6])``r abs(Hnj[3,6])`x^2,\\
H_{3,3}(x)=&(1-2 (x-`r x[4]`) L_{n,3}'(`r x[4]`))L_{n,3}^2 (x)=(1-2 (x-`r x[4]`) (`r round(horner(pols.lagrange[4,],x[4])$dy,4)`))\left(\frac{1}{`r denoms[4]`}\cdot (x^3`r signe(polis.lag[4,2])``r abs(polis.lag[4,2])` x^2`r signe(polis.lag[4,3])``r abs(polis.lag[4,3])`x)\right)^2\\ = & `r Hnj[4,1]` x^7`r signe(Hnj[4,2])``r abs(Hnj[4,2])`x^6`r signe(Hnj[4,3])``r abs(Hnj[4,3])`x^5`r signe(Hnj[4,4])``r abs(Hnj[4,4])`x^4`r signe(Hnj[4,5])``r abs(Hnj[4,5])`x^3`r signe(Hnj[4,6])``r abs(Hnj[4,6])`x^2,\\
\end{align*}
$$
</div>
...
$$
\begin{align*}
\hat{H}_{3,0}(x)=&x L_{n,0}^2 (x)\\ = &   `r Htildenj[1,1]` x^7`r signe(Htildenj[1,2])``r abs(Htildenj[1,2])`x^6`r signe(Htildenj[1,3])``r abs(Htildenj[1,3])`x^5`r signe(Htildenj[1,4])``r abs(Htildenj[1,4])`x^4`r signe(Htildenj[1,5])``r abs(Htildenj[1,5])`x^3`r signe(Htildenj[1,6])``r abs(Htildenj[1,6])`x^2`r signe(Htildenj[1,7])``r abs(Htildenj[1,7])`x,\\
\hat{H}_{3,1}(x)=&(x-`r x[2]`) L_{n,1}^2 (x) \\ = & `r Htildenj[2,1]` x^7`r signe(Htildenj[2,2])``r abs(Htildenj[2,2])`x^6`r signe(Htildenj[2,3])``r abs(Htildenj[2,3])`x^5`r signe(Htildenj[2,4])``r abs(Htildenj[2,4])`x^4`r signe(Htildenj[2,5])``r abs(Htildenj[2,5])`x^3`r signe(Htildenj[2,6])``r abs(Htildenj[2,6])`x^2,\\
\hat{H}_{3,2}(x)=& (x-`r x[3]`) L_{n,2}^2 (x)\\ = &   `r Htildenj[3,1]` x^7`r signe(Htildenj[3,2])``r abs(Htildenj[3,2])`x^6`r signe(Htildenj[3,3])``r abs(Htildenj[3,3])`x^5`r signe(Htildenj[3,4])``r abs(Htildenj[3,4])`x^4`r signe(Htildenj[3,5])``r abs(Htildenj[3,5])`x^3`r signe(Htildenj[3,6])``r abs(Htildenj[3,6])`x^2,\\
\hat{H}_{3,3}(x)=& (x-`r x[4]`) L_{n,3}^2 (x)\\ = &  `r Htildenj[4,1]` x^7`r signe(Htildenj[4,2])``r abs(Htildenj[4,2])`x^6`r signe(Htildenj[4,3])``r abs(Htildenj[4,3])`x^5`r signe(Htildenj[4,4])``r abs(Htildenj[4,4])`x^4`r signe(Htildenj[4,5])``r abs(Htildenj[4,5])`x^3`r signe(Htildenj[4,6])``r abs(Htildenj[4,6])`x^2,\\
\end{align*}
$$
</div>
...
$$
\begin{align*}
P_{7}(x) = &  `r y[1]`H_{3,0}(x)`r signe(y[2])``r abs(y[2])` H_{3,1}(x)`r signe(y[3])`
`r abs(y[3])` H_{3,2}(x)`r signe(y[4])``r abs(y[4])`H_{3,3}(x)+`r signe(y.der[1])``r abs(y.der[1])`\hat{H}_{3,0}(x)`r signe(y.der[2])``r abs(y.der[2])`\hat{H}_{3,1}(x)`r signe(y.der[3])``r abs(y.der[3])`\hat{H}_{3,2}(x)`r signe(y.der[4])``r abs(y.der[4])`\hat{H}_{3,3}(x)\\ = & `r signe(y[2])``r abs(y[2])` (`r Hnj[2,1]` x^7`r signe(Hnj[2,2])``r abs(Hnj[2,2])`x^6`r signe(Hnj[2,3])``r abs(Hnj[2,3])`x^5`r signe(Hnj[2,4])``r abs(Hnj[2,4])`x^4`r signe(Hnj[2,5])``r abs(Hnj[2,5])`x^3`r signe(Hnj[2,6])``r abs(Hnj[2,6])`x^2)\\
 & `r signe(y[3])` `r abs(y[3])` (`r Hnj[3,1]` x^7`r signe(Hnj[3,2])``r abs(Hnj[3,2])`x^6`r signe(Hnj[3,3])``r abs(Hnj[3,3])`x^5`r signe(Hnj[3,4])``r abs(Hnj[3,4])`x^4`r signe(Hnj[3,5])``r abs(Hnj[3,5])`x^3`r signe(Hnj[3,6])``r abs(Hnj[3,6])`x^2)\\
 & `r signe(y[4])` `r abs(y[4])` (`r Hnj[4,1]` x^7`r signe(Hnj[4,2])``r abs(Hnj[4,2])`x^6`r signe(Hnj[4,3])``r abs(Hnj[4,3])`x^5`r signe(Hnj[4,4])``r abs(Hnj[4,4])`x^4`r signe(Hnj[4,5])``r abs(Hnj[4,5])`x^3`r signe(Hnj[4,6])``r abs(Hnj[4,6])`x^2)\\ & `r signe(y.der[2])``r abs(y.der[2])`(`r Htildenj[2,1]` x^7`r signe(Htildenj[2,2])``r abs(Htildenj[2,2])`x^6`r signe(Htildenj[2,3])``r abs(Htildenj[2,3])`x^5`r signe(Htildenj[2,4])``r abs(Htildenj[2,4])`x^4`r signe(Htildenj[2,5])``r abs(Htildenj[2,5])`x^3`r signe(Htildenj[2,6])``r abs(Htildenj[2,6])`x^2)
 \\ & `r signe(y.der[3])``r abs(y.der[3])` (`r Htildenj[3,1]` x^7`r signe(Htildenj[3,2])``r abs(Htildenj[3,2])`x^6`r signe(Htildenj[3,3])``r abs(Htildenj[3,3])`x^5`r signe(Htildenj[3,4])``r abs(Htildenj[3,4])`x^4`r signe(Htildenj[3,5])``r abs(Htildenj[3,5])`x^3`r signe(Htildenj[3,6])``r abs(Htildenj[3,6])`x^2)\\ & `r signe(y.der[4])``r abs(y.der[4])` (`r Htildenj[4,1]` x^7`r signe(Htildenj[4,2])``r abs(Htildenj[4,2])`x^6`r signe(Htildenj[4,3])``r abs(Htildenj[4,3])`x^5`r signe(Htildenj[4,4])``r abs(Htildenj[4,4])`x^4`r signe(Htildenj[4,5])``r abs(Htildenj[4,5])`x^3`r signe(Htildenj[4,6])``r abs(Htildenj[4,6])`x^2) \\ = & `r p.hermite[1]` x^7`r signe(p.hermite[2])``r abs(p.hermite[2])`x^6`r signe(p.hermite[3])``r abs(p.hermite[3])`x^5`r signe(p.hermite[4])``r abs(p.hermite[4])`x^4`r signe(p.hermite[5])``r abs(p.hermite[5])`x^3`r signe(p.hermite[6])``r abs(p.hermite[6])`x^2.
\end{align*}
$$
...
<div class="center">
```{r, echo=FALSE, label=urna44,out.width = "550px"}
knitr::include_graphics("04Interpolacion_files/hermite.png",dpi=400)
```
</div>
...
$$
\begin{align*}
P_7(x)= & `r Q[1,1]`+`r Q[2,2]`(x-`r x[1]`)+`r Q[3,3]`(x-`r x[1]`)^2 `r Q[4,4]` (x-`r x[1]`)^2 (x-`r x[2]`)+`r round(Q[5,5],3)` (x-`r x[1]`)^2 (x-`r x[2]`)^2 \\ & +`r round(Q[6,6],3)` (x-`r x[1]`)^2 (x-`r x[2]`)^2 (x-`r x[3]`) `r round(Q[7,7],3)` (x-`r x[1]`)^2 (x-`r x[2]`)^2 (x-`r x[3]`)^2\\ & +`r round(Q[8,8],3)` (x-`r x[1]`)^2 (x-`r x[2]`)^2 (x-`r x[3]`)^2 (x-`r x[4]`)\\ = & x^2 - x^2 (x-`r x[2]`)+`r round(Q[5,5],3)` x^2 (x-`r x[2]`)^2  +`r round(Q[6,6],3)` x^2 (x-`r x[2]`)^2 (x-`r x[3]`) `r round(Q[7,7],3)` x^2 (x-`r x[2]`)^2 (x-`r x[3]`)^2\\ & +`r round(Q[8,8],3)` x^2 (x-`r x[2]`)^2 (x-`r x[3]`)^2 (x-`r x[4]`)
\end{align*}
$$
...
f=function(t){1/(1+25*t^2)}
bb = function(s,k){
  if(k==0){return(1)}
  if(k!=0){return(prod(s-(0:(k-1)))/factorial(k))}
}

xx=seq(from=-1,to=1,by=0.01)
resul=list()
...
plot(c(xmin-tolx,xmax+tolx,xmin-tolx,xmax+tolx),c(ymin-50*toly,ymin-50*toly,ymax+50*toly,ymax+50*toly),type="n",xlab="",ylab="",xaxt="n",yaxt="n",axes=FALSE)
x.plot=seq(from=xmin,to=xmax,by=0.01)
#points(x,f(x),type="l")
...
lines(xx,resul[[1]])
lines(xx,resul[[2]],col="blue")
lines(xx,resul[[3]],col="green")
```
</div>
...
```{r,echo=FALSE}
matriz=matrix(c(1,1,0,0,0,0,0,2,4,8,1,3,-1,0,0,0,3,0,-1,0,0,0,0,1,6),5,5,byrow=TRUE)
b=c(1,-4,0,0,0)
x=solve(matriz,b)
```
...
$$
b_0=`r x[1]`,\ d_0=`r x[2]`,\ b_1=`r x[3]`,\ c_1=`r x[4]`,\ d_1=`r x[5]`.
$$
Los polinomios $S_0(x)$ y $S_1(x)$ son los siguientes:
$$
S_0(x)= `r x[1]` x`r signe(x[2])``r abs(x[2])` x^3,\quad
S_1(x)= 1`r signe(x[4])``r abs(x[4])`(x-1)^2`r signe(x[5])``r abs(x[5])`(x-1)^3.
$$
...
quantsy=4
f=function(x){x*sin(pi*x/2)}
S0=function(x){1.5*x-0.5*x^3}
S1=function(x){1-1.5*(x-1)^2+0.25*(x-1)^3}
plot(c(xmin-tolx,xmax+tolx,xmin-tolx,xmax+tolx),c(ymin-50*toly,ymin-50*toly,ymax+50*toly,ymax+50*toly),type="n",xlab="",ylab="",xaxt="n",yaxt="n",axes=FALSE)
x.plot=seq(from=xmin,to=xmax,by=0.001)
lines(c(0,0),c(ymin,ymax))
lines(c(xmin,xmax),c(0,0))
for (i in 0:(quantsx)){
  lines(rep(xmin+((xmax-xmin)/quantsx)*i,2),c(-0.5*tolx,+0.5*tolx))
  text(xmin+((xmax-xmin)/quantsx)*i,-50*toly,xmin+((xmax-xmin)/quantsx)*i,cex=0.75)
}

for (i in 0:(quantsy)){
  if(i!=3){
  lines(c(-0.25*tolx,0.25*tolx),ymin+rep(((ymax-ymin)/quantsy)*i,2))
  text(0.75*tolx,ymin+((ymax-ymin)/quantsy)*i,ymin+((ymax-ymin)/quantsy)*i,cex=0.75)
  }
}
lines(x.plot,f(x.plot),col="black")
x0=seq(from=0,to=1,by=0.001)
lines(x0,S0(x0),col="red")
x1=seq(from=1,to=3,by=0.001)
lines(x1,S1(x1),col="blue")
points(c(0,1,3),c(0,1,-3),pch=19)
```
</div>
</div>
...
```{r,echo=FALSE}
n=5
x=seq(from=0,to=3,by=3/n)
f=function(x){x*sin(pi*x/2)}
h=x[-1]-x[1:(length(x)-1)]
as=f(x)
y=(3/h[-c(1,n+1)])*(as[-c(1,2)]-as[-c(1,(n+1))])-(3/h[-c(n,n+1)])*(as[-c(1,(n+1))]-as[-c(n,n+1)])
A=matrix(0,n-1,n-1)
A[1,1]=2*(h[1]+h[2])
A[1,2]=h[2]
for (i in 2:(n-2)){
...
cs=solve(A,y)
cs=c(0,cs,0)
bs=(1/h)*(as[-1]-as[-length(as)])-(h/3)*(cs[-1]+2*cs[-length(cs)])
ds=(1/(3*h))*(cs[-1]-cs[-length(cs)])
```
<div class="example">
...
$`r x`$ y los puntos por los que debe pasar el spline son:

<div class="center">
|$x_i$|$`r x[1]`$|$`r x[2]`$|$`r x[3]`$|$`r x[4]`$|$`r x[5]`$|$`r x[6]`$|
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
|$a_i=f(x_i)$|$`r f(x[1])`$|$`r f(x[2])`$|$`r f(x[3])`$|$`r f(x[4])`$|$`r f(x[5])`$|$`r f(x[6])`$|
</div>
...
Los valores $h_i$ son constantes e iguales a $h_i=`r h[1]`$.
...
$$
\begin{align*}
y_1 = & \frac{3}{h_1}(a_2-a_1)-\frac{3}{h_0}(a_1-a_0)=\frac{3}{`r h[2]`}(`r as[3]`-`r as[2]`)-\frac{3}{`r h[1]`}(`r as[2]`-`r as[1]`)\\ = & `r y[1]`,\\
y_2 = & \frac{3}{h_2}(a_3-a_2)-\frac{3}{h_1}(a_2-a_1)=\frac{3}{`r h[3]`}(`r as[4]`-`r as[3]`)-\frac{3}{`r h[2]`}(`r as[3]`-`r as[2]`)\\ = & `r y[2]`,\\
y_3 = & \frac{3}{h_3}(a_4-a_3)-\frac{3}{h_2}(a_3-a_2)=\frac{3}{`r h[4]`}(`r as[5]`-`r as[4]`)-\frac{3}{`r h[3]`}(`r as[4]`-`r as[3]`)\\ = & `r y[3]`,\\
y_4 = & \frac{3}{h_4}(a_5-a_4)-\frac{3}{h_3}(a_4-a_3)=\frac{3}{`r h[5]`}(`r as[6]`-`r as[5]`)-\frac{3}{`r h[4]`}(`r as[5]`-`r as[4]`)\\ = & `r y[4]`.
\end{align*}
$$
...
`r 2*(h[1]+h[2])`&`r h[2]`&0&0\\
`r h[2]`&`r 2*(h[2]+h[3])`&`r h[3]`&0\\
0&`r h[3]`&`r 2*(h[3]+h[4])`&`r h[4]`\\
0&0&`r h[4]`&`r 2*(h[4]+h[5])`
\end{pmatrix}
$$
La solución del sistema $\mathbf{A}\mathbf{c}=\mathbf{y}$ es la siguiente:
$$
\mathbf{c}=(c_1,c_2,c_3,c_4)=(`r cs[-c(1,length(cs))]`).
$$
Usando que $c_0=c_5=0$, tenemos que los coeficientes $c_i$ son los siguientes:
$$
\mathbf{c}=(c_0,c_1,c_2,c_3,c_4,c_5)=(`r cs`).
$$
</div>
...
$$
\begin{align*}
b_0 = & \frac{1}{h_0}(a_1-a_0)-\frac{h_0}{3}(c_1+2c_0)=\frac{1}{`r h[1]`}(`r as[2]`-`r as[1]`)-\frac{`r h[1]`}{3}(`r cs[2]`+2\cdot `r cs[1]`)=`r bs[1]`,\\
b_1 = & \frac{1}{h_1}(a_2-a_1)-\frac{h_1}{3}(c_2+2c_1)\\ = & \frac{1}{`r h[2]`}(`r as[3]`-`r as[2]`)-\frac{`r h[2]`}{3}(`r cs[3]`+2\cdot `r cs[2]`)=`r bs[2]`,\\
b_2 = & \frac{1}{h_2}(a_3-a_2)-\frac{h_2}{3}(c_3+2c_2)\\ = & \frac{1}{`r h[3]`}(`r as[4]`-`r as[3]`)-\frac{`r h[3]`}{3}(`r cs[4]`+2\cdot (`r cs[3]`))=`r bs[3]`,\\
b_3 = & \frac{1}{h_3}(a_4-a_3)-\frac{h_3}{3}(c_4+2c_3)\\ = & \frac{1}{`r h[4]`}(`r as[5]`-`r as[4]`)-\frac{`r h[4]`}{3}(`r cs[5]`+2\cdot (`r cs[4]`))=`r bs[4]`,\\
b_4 = & \frac{1}{h_4}(a_5-a_4)-\frac{h_4}{3}(c_5+2c_4)\\ = & \frac{1}{`r h[5]`}(`r as[6]`-(`r as[5]`))-\frac{`r h[5]`}{3}(`r cs[6]`+2\cdot `r cs[5]`)=`r bs[5]`.
\end{align*}
$$
...
$$
\begin{align*}
d_0 = & \frac{c_1-c_0}{3h_0}=\frac{`r cs[2]`-`r cs[1]`}{3\cdot `r h[1]`}=`r ds[1]`,\\
d_1 = & \frac{c_2-c_1}{3h_1}=\frac{`r cs[3]`-`r cs[2]`}{3\cdot `r h[2]`}=`r ds[2]`,\\
d_2 = & \frac{c_3-c_2}{3h_2}=\frac{`r cs[4]`-(`r cs[3]`)}{3\cdot `r h[3]`}=`r ds[3]`,\\
d_3 = & \frac{c_4-c_3}{3h_3}=\frac{`r cs[5]`-(`r cs[4]`)}{3\cdot `r h[4]`}=`r ds[4]`,\\
d_4 = & \frac{c_5-c_4}{3h_4}=\frac{`r cs[6]`-`r cs[5]`}{3\cdot `r h[5]`}=`r ds[5]`,\\
\end{align*}
$$
...
S(x)=
\begin{cases}
S_0(x)= & `r bs[1]` x+`r ds[1]` x^3,\ \mbox{ si }x\in [`r x[1]`,`r x[2]`],\\
S_1(x)= & `r as[2]``r signe(bs[2])``r abs(bs[2])`(x-`r x[2]`)`r signe(cs[2])``r abs(cs[2])`(x-`r x[2]`)^2`r signe(ds[2])``r abs(ds[2])`(x-`r x[2]`)^3,\\ &  \mbox{ si }x\in [`r x[2]`,`r x[3]`],\\
S_2(x)= & `r as[3]``r signe(bs[3])``r abs(bs[3])`(x-`r x[3]`)`r signe(cs[3])``r abs(cs[3])`(x-`r x[3]`)^2`r signe(ds[3])``r abs(ds[3])`(x-`r x[3]`)^3,\\ &  \mbox{ si }x\in [`r x[3]`,`r x[4]`],\\
S_3(x)= & `r as[4]``r signe(bs[4])``r abs(bs[4])`(x-`r x[4]`)`r signe(cs[4])``r abs(cs[4])`(x-`r x[4]`)^2`r signe(ds[4])``r abs(ds[4])`(x-`r x[4]`)^3,\\ &  \mbox{ si }x\in [`r x[4]`,`r x[5]`],\\
S_4(x)= & `r as[5]``r signe(bs[5])``r abs(bs[5])`(x-`r x[5]`)`r signe(cs[5])``r abs(cs[5])`(x-`r x[5]`)^2`r signe(ds[5])``r abs(ds[5])`(x-`r x[5]`)^3, \\ &  \mbox{ si }x\in [`r x[5]`,`r x[6]`].
\end{cases}
$$
...
<div class="example">
<div class="center">
```{r,echo=FALSE,fig=TRUE,fig.height=6.5,fig.width=8}
nodes=seq(from=0,to=3,by=3/n)
xmin=0
xmax=3
ymin=-3
ymax=2
...
quantsy=5
f = function(x){x*sin(pi*x/2)}
S0 = function(x){as[1]+bs[1]*(x-x[1])+cs[1]*(x-x[1])^2+ds[1]*(x-x[1])^3}
S1 = function(x){as[2]+bs[2]*(x-x[2])+cs[2]*(x-x[2])^2+ds[2]*(x-x[2])^3}
S2 = function(x){as[3]+bs[3]*(x-x[3])+cs[3]*(x-x[3])^2+ds[3]*(x-x[3])^3}
S3 = function(x){as[4]+bs[4]*(x-x[4])+cs[4]*(x-x[4])^2+ds[4]*(x-x[4])^3}
S4 = function(x){as[5]+bs[5]*(x-x[5])+cs[5]*(x-x[5])^2+ds[5]*(x-x[5])^3}
plot(c(xmin-tolx,xmax+tolx,xmin-tolx,xmax+tolx),c(ymin-50*toly,ymin-50*toly,ymax+50*toly,ymax+50*toly),type="n",xlab="",ylab="",xaxt="n",yaxt="n",axes=FALSE)
x.plot=seq(from=xmin,to=xmax,by=0.01)
xx=list()
for (i in 1:(length(nodes)-1)){
  xx[[i]]=seq(from=nodes[i],to=nodes[i+1],by=0.001)
}
lines(c(0,0),c(ymin,ymax))
lines(c(xmin,xmax),c(0,0))
for (i in 0:(quantsx)){
  if(i!=0){
  lines(rep(xmin+((xmax-xmin)/quantsx)*i,2),c(-5*tolx,5*tolx))
  text(xmin+((xmax-xmin)/quantsx)*i,-50*toly,xmin+((xmax-xmin)/quantsx)*i,cex=0.75)
  }
}
...
lines(x.plot,f(x.plot),col="black")
lines(xx[[1]],S0(xx[[1]]),col="blue")
lines(xx[[2]],S1(xx[[2]]),col="red")
lines(xx[[3]],S2(xx[[3]]),col="blue")
lines(xx[[4]],S3(xx[[4]]),col="red")
lines(xx[[5]],S4(xx[[5]]),col="blue")
```
</div>
</div>
...
h=x[-1]-x[1:(length(x)-1)]
as=f(x)
y=c(3*(as[2]-as[1])/h[1],(3/h[-c(1,n+1)])*(as[-c(1,2)]-as[-c(1,(n+1))])-
      (3/h[-c(n,n+1)])*(as[-c(1,(n+1))]-as[-c(n,n+1)]),-3-3*(as[n+1]-as[n])/h[n])
A=matrix(0,n+1,n+1)
A[1,1]=2*h[1]
A[1,2]=h[1]
for (i in 2:n){
...
```
<div class="example">
Vamos a hallar el **spline interpolador de frontera fija** con la función anterior $f(x)=x\cdot \sin\left(\frac{\pi}{2}x\right)$ y los nodos equiespaciados anteriores en el intervalo $[0,3]$, $x_i=\frac{3i}{n}$, $i=0,\ldots,n$ con $n=5$: $`r x`$.
...
Recordemos que los valores $h_i$ son constantes e iguales a $h_i=`r h[1]`$.

En este caso debemos calcular $f'(`r x[1]`)$ y $f'(`r x[6]`)$. La función $f'(x)$ vale:
$$
f'(x)=\sin \left(\frac{\pi  x}{2}\right)+\frac{1}{2} \pi  x \cos
   \left(\frac{\pi  x}{2}\right).
$$
Por tanto, $f'(`r x[1]`)=0$ y $f'(`r x[6]`)=-1$.
...
$$
\begin{align*}
y_0 = & \frac{3(a_1-a_0)}{h_0}=\frac{3\cdot (`r as[2]`-`r as[1]`)}{`r h[1]`}=`r y[1]`,\\
y_1 = & \frac{3}{h_1}(a_2-a_1)-\frac{3}{h_0}(a_1-a_0)=\frac{3}{`r h[2]`}(`r as[3]`-`r as[2]`)-\frac{3}{`r h[1]`}(`r as[2]`-`r as[1]`)\\ = & `r y[2]`,\\
y_2 = & \frac{3}{h_2}(a_3-a_2)-\frac{3}{h_1}(a_2-a_1)=\frac{3}{`r h[3]`}(`r as[4]`-`r as[3]`)-\frac{3}{`r h[2]`}(`r as[3]`-`r as[2]`)\\ = & `r y[3]`,\\
y_3 = & \frac{3}{h_3}(a_4-a_3)-\frac{3}{h_2}(a_3-a_2)=\frac{3}{`r h[4]`}(`r as[5]`-`r as[4]`)-\frac{3}{`r h[3]`}(`r as[4]`-`r as[3]`)\\ = & `r y[4]`,\\
y_4 = & \frac{3}{h_4}(a_5-a_4)-\frac{3}{h_3}(a_4-a_3)=\frac{3}{`r h[5]`}(`r as[6]`-(`r as[5]`))-\frac{3}{`r h[4]`}(`r as[5]`-`r as[4]`)\\ = & `r y[5]`,\\
y_5 = & -3-\frac{3(a_n-a_{n-1})}{h_{n-1}}=-3-\frac{3\cdot (`r as[n+1]`-(`r as[n]`))}{`r h[n]`}=`r y[6]`.
\end{align*}
$$
...
\end{pmatrix}\\
= & \begin{pmatrix}
`r 2*h[1]`&`r h[1]`&0&0&0&0\\
`r h[1]`& `r 2*(h[1]+h[2])`&`r h[2]`&0&0&0\\
0&`r h[2]`&`r 2*(h[2]+h[3])`&`r h[3]`&0&0\\
0&0&`r h[3]`&`r 2*(h[3]+h[4])`&`r h[4]`&0\\
0&0&0&`r h[4]`&`r 2*(h[4]+h[5])`&`r h[5]`\\
0&0&0&0&`r h[5]`&`r 2*h[5]`
\end{pmatrix}
\end{align*}
$$
...
S(x)=
\begin{cases}
S_0(x)= & `r cs[1]` x^2`r signe(ds[1])``r abs(ds[1])` x^3,\ \mbox{ si }x\in [`r x[1]`,`r x[2]`],\\
S_1(x)= & `r as[2]``r signe(bs[2])``r abs(bs[2])`(x-`r x[2]`)`r signe(cs[2])``r abs(cs[2])`(x-`r x[2]`)^2`r signe(ds[2])``r abs(ds[2])`(x-`r x[2]`)^3,\\ &  \mbox{ si }x\in [`r x[2]`,`r x[3]`],\\
S_2(x)= & `r as[3]``r signe(bs[3])``r abs(bs[3])`(x-`r x[3]`)`r signe(cs[3])``r abs(cs[3])`(x-`r x[3]`)^2`r signe(ds[3])``r abs(ds[3])`(x-`r x[3]`)^3,\\ &  \mbox{ si }x\in [`r x[3]`,`r x[4]`],\\
S_3(x)= & `r as[4]``r signe(bs[4])``r abs(bs[4])`(x-`r x[4]`)`r signe(cs[4])``r abs(cs[4])`(x-`r x[4]`)^2`r signe(ds[4])``r abs(ds[4])`(x-`r x[4]`)^3,\\ &  \mbox{ si }x\in [`r x[4]`,`r x[5]`],\\
S_4(x)= & `r as[5]``r signe(bs[5])``r abs(bs[5])`(x-`r x[5]`)`r signe(cs[5])``r abs(cs[5])`(x-`r x[5]`)^2`r signe(ds[5])``r abs(ds[5])`(x-`r x[5]`)^3, \\ &  \mbox{ si }x\in [`r x[5]`,`r x[6]`].
